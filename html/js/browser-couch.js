...
var createUUID = new UUID().createUUID,
    json       = JSON.parse,
    stringify  = JSON.stringify,
    storage    = new LocalStorage()

function BrowserCouch(name){
...
//
  ...
  this.allDocs = function(options,keys){
    return storage.allDocs(docPrefix,options,keys)
  }
...
//    if(!doc._id) doc._id = random_id()
//    storage.save(doc, options)
//  }
  ...
  this.save = function(doc, options){
    ...
    var putObj = function(obj){
      ...
      if(newEdits && orig && orig._rev != obj._rev){
        ...
        _lastSeq = seq;
        storage.save(docPrefix + obj._id, obj)
        storage.save(seqPrefix + seq, obj._id)
      }
    }
...
////    }
  ...
  this.deleteDoc = function(doc) {
    return this.save({_id : doc._id, _rev : doc._rev, _deleted : true})
  }
...
BrowserCouch.getVersion = function(){
  return BrowserCouch.info().version
}
...
function FakeStorage(){
  ...
  this.allDocs = function(options,cb){
    // TODO
    cb({})
  }
...
// future we may want to rethink this as it's horribly inefficient
...
function LocalStorage() {
  ...
  this.get = function(name) {
    if (name in storage && storage[name])
      return json(storage[name])
    else
      return null
  };

//  this.put = function(name, obj, cb) {
  this.save = function(name, doc, options) {
    return storage[name] = stringify(doc)
  };
  ...
  this.allDocs = function(prefix, options, keys){
    var keys = self.keys(prefix)
    return({
  ...
  this.allDbs = function(){
    var re = /BC_DB_([a-z]+)__?[a-z]+__?\d+/
    var dbs = []
    for(var i = 0; i<storage.length; i++){
      var match = re.exec(storage.key(i))
      if(match && match.length > 0){
        var db = match[1]
        if(db && dbs.indexOf(db) == -1) dbs.push(db)
      }
    }
    return dbs
  }
...
BrowserCouch.request = function(method, uri, options) {
  throw "NotImplementedError"
}

// === //List All Databases// ===

BrowserCouch.allDbs = function(){
  return storage.allDbs()
}

BrowserCouch.uuids_cache = [];

BrowserCouch.newUuids = function(n, buf) {
  buf = buf || 100;
  if (CouchDB.uuids_cache.length >= n) {
    ...
    CouchDB.uuids_cache = CouchDB.uuids_cache.concat(uuids.slice(0, buf))
    return uuids.slice(buf)
  }
}

// Compatibility
var CouchDB = BrowserCouch